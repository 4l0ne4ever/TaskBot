...
// backend/app/utils/recurrence.py for the whitelist). This component
// renders Vietnamese-leaning labels because the dogfood persona is VN.
//
...
function formatByday(value: string): string {
  return value
    ...
    .map((tok) => {
      ...
      if (!m) return t;
      const label = VN_WEEKDAY[m[2]] ?? m[2];
...
export function formatRecurrence(rrule: string | null | undefined): string {
  ...
  const intervalLabel = interval > 1 ? `mỗi ${interval} ` : "";
  let base = "";
  if (freq === "DAILY") {
    base = interval > 1 ? `${intervalLabel}ngày` : "Hằng ngày";
  } else if (freq === "WEEKLY") {
    const days = kv.BYDAY ? formatByday(kv.BYDAY) : "";
    base = days
      ? interval > 1
        ? `${intervalLabel}tuần — ${days}`
        : `Hằng tuần — ${days}`
      : interval > 1
        ? `${intervalLabel}tuần`
        : "Hằng tuần";
    ...
    if (kv.BYMONTHDAY) {
      base = interval > 1 ? `${intervalLabel}tháng — ngày ${kv.BYMONTHDAY}` : `Hằng tháng — ngày ${kv.BYMONTHDAY}`;
    } else if (kv.BYDAY) {
      base = interval > 1 ? `${intervalLabel}tháng — ${formatByday(kv.BYDAY)}` : `Hằng tháng — ${formatByday(kv.BYDAY)}`;
    } else {
      base = interval > 1 ? `${intervalLabel}tháng` : "Hằng tháng";
    }
  } else if (freq === "YEARLY") {
    base = interval > 1 ? `${intervalLabel}năm` : "Hằng năm";
  } else {
...
export function RecurrenceBadge({
  ...
  const prefix = variant === "suggested" ? "💡 Gợi ý lặp lại: " : "🔁 ";
  return (
    <span
