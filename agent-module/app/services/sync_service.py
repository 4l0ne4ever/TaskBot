...
async def pull_recent_gmail_messages(
    *,
    ...
    messages = await gmail.list_messages(last_sync_at=last_sync_at, max_results=50)
    if messages:
...
async def pull_recent_drive_files(
    *,
    ...
    owned = await drive.list_files(last_sync_at=last_sync_at, page_size=50)
    shared = await drive.list_shared_files(last_sync_at=last_sync_at)
