...
def _get_existing_tasks(state: PipelineState) -> list[dict]:
    # Temporary source for Phase 2.5. In later phases, this should be loaded from DB query by user_id.
    existing = state.get("existing_tasks")
...
def validate_tasks(state: PipelineState) -> dict:
    ...
    for task in normalized:
        ...
        similar_candidates = [
            ...
            and _similarity(title, str(candidate.get("title") or "")) >= TITLE_SIMILARITY_THRESHOLD
        ]
        try:
